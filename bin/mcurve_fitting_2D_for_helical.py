...
def find_seed(i, j, X, Y, Z):
    ...
    while True:
        ...
        for k in range(0, total_number, 1):
            ...
            if abs(dis_to_line) < max_distance_to_line:
                min_dis = 100000
                for index in tmp_Z:
                    dis_index_k = distance_two_points(
                        X[index], Y[index], X[k], Y[k])
...
def angle_evaluate(poly, point, mode):
    ...
    if mode == 1:
        ...
        while accumulation < evaluation_step:
            next_x = current_x + intergration_step
            next_y = poly(next_x)
            dist = distance_two_points(current_x, current_y, next_x, next_y)
            accumulation += dist
            current_x = next_x
            current_y = next_y
        next_x = current_x + intergration_step
...
def resample(poly, left, right, mode, sample_step, cluster):
    ...
    if mode == 1:  # y=f(x)
        ...
        output_resam_Zscore.write(str(cluster+1) + " ")
        output_resam_Zscore.write("90.000000" + " ")
        output_resam_Zscore.write(str(angle) + " ")
        output_resam_Zscore.write(str(0) + " ")
        ...
        while current_x < right:
            ...
            dist = distance_two_points(current_x, current_y, next_x, next_y)
            accumulation += dist
            if (sample_step - accumulation) < 0.05:
                accumulation = 0
                slope = (next_y - current_y)/(next_x - current_x)
                angle = math.degrees(math.atan(slope))
                angle = - angle
                ...
                output_resam_Zscore.write("90.000000" + " ")
                output_resam_Zscore.write(str(angle) + " ")
                output_resam_Zscore.write(str(count*sample_step) + " ")
                output_resam_Zscore.write("0.500000" + "\n")
                count += 1
                output_resam_Zscore_txt.write(str(current_x) + " ")
                output_resam_Zscore_txt.write(str(current_y) + " ")
        ...
        angle_tmp = math.degrees(math.atan(slope))
        if angle_tmp >= 0:
            angle = 90 - angle_tmp
            flag = 1
        else:
...
def seed_extension(tmp_Z, X, Y, Z, cluster):
    ...
    if delta_x >= delta_y:  # y=f(x)
        ...
        while True:
            ...
            for k in range(0, total_number, 1):
                ...
                if dis_to_curve < max_distance_to_curve:
                    min_dis = 100000
                    for index in tmp_Z:
                        dis_index_k = distance_two_points(
                            X[index], Y[index], X[k], Y[k])
    ...
    if (len(tmp_Z) - min_number) >= extension_min_number and seed_evaluation == 1 and seed_angle_evalution == 1:
        ...
        for index in tmp_Z:
            ...
            output_Zscore.write(str(cluster+1))
            output_Zscore.write("\n")
            output_Zscore_txt.write(str(X[index]) + " ")
            output_Zscore_txt.write(str(Y[index]) + " ")
            output_Zscore_txt.write(str(cluster+1))
            output_Zscore_txt.write("\n")
        if mode == 1:
            resample(poly, min(X_cluster), max(
                X_cluster), 1, sample_step, cluster)
        else:
            resample(poly, min(Y_cluster), max(
                Y_cluster), 0, sample_step, cluster)
        print("Done resampling! " + "cluster " + str(cluster+1) + "\n")
        return 1
    else:
        for index in tmp_Z:
            Z[index] = -1
...
def print_help_message():
    ...
    print("This program/script performs multi-curves fitting of the input coordinates (2D).")
    print("Supported file formats: relion star file format(end with .star) or 2-column txt file format(end with .txt)\n")
    print(color.RED + color.BOLD + "Some notes: " + color.END)
    print("Star file must include rlnCoordinateX rlnCoordinateY")
    ...
    print("The resampled file follows the format of relion helical extraction job.")
    ...
    print('{:30s} {:20s}'.format("arguments", "defulat values") +
          "decription")
    print('{:30s} {:20s}'.format("--pixel_size_ang", "1.33") +
    ...
    print('{:30s} {:20s}'.format("--max_seed_fitting_error", "4") +
...
arguments["max_seed_fitting_error"] = "4"
